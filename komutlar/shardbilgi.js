...
let i = message.guild.shardID
    ...
    for(i=0;i<client.shard.count;i++) {
        shardembed.addField(`<a:yukenliyo:781141588881309706> Shard: ${i+1} | <a:yukenliyo:781141588881309706> Ping: ${Math.round(shardinfo.ping[i])}ms `, ` <a:yukenliyo:781141588881309706> ${shardinfo.server_count[i]} sunucu ve ${shardinfo.user_count[i]} kullanıcı\n <a:yukenliyo:781141588881309706>Uptime: ${moment.duration(shardinfo.uptime[i]).format(`D [Gün] , H [Saat], m [Dakika], s [Saniye]`)} `)
    }
    ...
    const embed = new Discord.MessageEmbed()
      .setDescription(
        `Sanırım bir sorun var! Bunu yetkililere bildir!\nDestek Sunucusu: [Tıkla!](https://discord.gg/82HpFtq9B9) \nHata kodu: ${err}`
      )
      .setColor("RED")
      .setThumbnail(client.user.avatarURL())
      .setTimestamp();
